export async function getGraphBuildingForSubTree(path: string) {
  ...
  type Transform = [number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number];
  
  export function transformCoordinates(v: [number, number], t: Transform) {
    return [
      v[0] * t[0] + v[1] * t[4] + 1 * t[12],
      v[0] * t[1] + v[1] * t[5] + 1 * t[13]
    ]
  }
